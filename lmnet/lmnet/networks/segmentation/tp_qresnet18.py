...
def resnet_basicblock(name, x, ch_out, stride, data_format, activation):
...
def resnet_backbone(image, ch_out, data_format, activation, kernel_setter):
    x = fix_conv('Conv0', image, 32, 3, 2, data_format)
    # x = activation(fix_bn('bn0', x, data_format))
    with tf.variable_scope('qresidual', custom_getter=kernel_setter):
        # x = fix_conv('Conv1', x, ch_out, 1, 2, data_format)
        x = resnet_basicblock('block_1_1', x, ch_out, 2, data_format, activation)
        x = resnet_basicblock('block_1_2', x, ch_out, 1, data_format, activation)
        x = resnet_basicblock('block_2_1', x, ch_out * 2, 2, data_format, activation)
        x = resnet_basicblock('block_2_2', x, ch_out * 2, 1, data_format, activation)
        x = resnet_basicblock('block_3_1', x, ch_out * 4, 2, data_format, activation)
        x = resnet_basicblock('block_3_2', x, ch_out * 4, 1, data_format, activation)
        x16_down = x
        x = resnet_basicblock('block_4_1', x, ch_out * 8, 2, data_format, activation)
        x = resnet_basicblock('block_4_2', x, ch_out * 8, 1, data_format, activation)
    # x = tf.nn.relu(x)
    x = tf.nn.relu(fix_bn('gbn', x, data_format))
    x32_down = x
